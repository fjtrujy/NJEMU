...
/******************************************************************************
	Always-compiled functions (no GUI dependency)
******************************************************************************/
...
void find_state_file(uint8_t *slot)
{
	...
	if (dp != NULL)
	{
		while ((entry = readdir(dp)) != NULL)
		{
			if (strncasecmp(entry->d_name, pattern, len) == 0)
			{
				int number = entry->d_name[len] - '0';
...
static struct file_entry *files[MAX_ENTRY];
...
/*--------------------------------------------------------
	Check if path is a device root (e.g. "ms0:/", "host0:/")
--------------------------------------------------------*/

static int is_device_root(const char *path)
{
	const char *colon = strchr(path, ':');
	return (colon && strcmp(colon, ":/") == 0);
}

static int is_bare_device(const char *path)
{
	const char *colon = strchr(path, ':');
...
static void getDir(const char *path)
{
	...
	if (dp != NULL)
	{
		while (nfiles < MAX_ENTRY)
		{
...
#if (EMU_SYSTEM == MVS)
			if (strcasecmp(entry->d_name, "neogeo.zip") == 0)
			{
				continue;
			}
#elif (EMU_SYSTEM == NCDZ)
			if (strcasecmp(entry->d_name, "ipl.txt") == 0)
			{
				neocddir = 1;
				continue;
			}
			...
			{
#ifdef COMMAND_LIST
				if (strcasecmp(entry->d_name, "command.dat") == 0)
				{
					strcpy(files[nfiles]->name, entry->d_name);
					strcpy(files[nfiles]->title, entry->d_name);
...
#endif
				if (strcasecmp(entry->d_name, "config") == 0) continue;
//				if (strcasecmp(entry->d_name, "snap") == 0) continue;
#if (EMU_SYSTEM != NCDZ)
				if (strcasecmp(entry->d_name, "nvram") == 0) continue;
#endif
#ifdef SAVE_STATE
				if (strcasecmp(entry->d_name, "state") == 0) continue;
#endif
#if (EMU_SYSTEM == MVS)
				if (strcasecmp(entry->d_name, "memcard") == 0) continue;
#elif (EMU_SYSTEM == NCDZ)
				if (strcasecmp(entry->d_name, "mp3") == 0)
				{
					has_mp3 = 1;
					continue;
				}
