/***************************************************************************
    ...
    Desktop PNG format image I/O functions. (based on M.A.M.E. PNG functions)
...
static int write_chunk(int fd, uint32_t chunk_type, uint8_t *chunk_data, uint32_t chunk_length)
{
	uint32_t crc;
	uint8_t v[4];
	uint32_t written;
...
static int png_write_sig(int fd)
{
	/* PNG Signature */
...
static int png_write_datastream(int fd, struct png_info *p)
{
	uint8_t ihdr[13];
	struct png_text *pt;
...
static int png_create_datastream(int fd)
{
	uint32_t x, y;
	uint8_t *dst;
	struct png_info p;
...
int save_png(const char *path)
{
	...
	if ((fd = open(path, O_WRONLY|O_CREAT, 0777)) >= 0)
	{
		...
		{
			if ((res = png_add_text("System", "Desktop")))
			{
